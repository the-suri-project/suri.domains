...
export class WalletStore {
    ...
    // Methods.
    signTransaction: ComputedRef<
        SignerWalletAdapter['signTransaction'] | undefined
    >;
    signAllTransactions: ComputedRef<
        SignerWalletAdapter['signAllTransactions'] | undefined
    >;
    signMessage: ComputedRef<
        MessageSignerWalletAdapter['signMessage'] | undefined
    >;
    ...
    constructor(props: WalletStoreProps) {
        ...
        this.storage = useLocalStorage(
            props.localStorageKey || 'selectedWallet',
            null
        );

        // Data.
        this.wallet = shallowRef<Wallet | null>(null);
        this.publicKey = ref(null);
        this.readyState = ref<WalletReadyState>(WalletReadyState.NotDetected);
        this.ready = computed(
            () =>
                this.readyState.value === WalletReadyState.Installed ||
                this.readyState.value === WalletReadyState.Loadable
        );
        ...
        // Private data.
        this.onError =
            props.onError || ((error: WalletError) => console.error(error));
        ...
        const onConnect = () => {
            ...
            this.publicKey.value = this.wallet.value?.publicKey ?? null;
        };
        const onDisconnect = () => {
            this.disconnected.value = true;
            this.connected.value = false;
            this.publicKey.value = null;
        };
    ...
    async sendTransaction(
        ...
        return await this.wallet.value.sendTransaction(
            transaction,
            connection,
            options
        );
...
function setWallet(newWallet: Wallet | null, self: WalletStore) {
    ...
    self.readyState.value =
        newWallet?.readyState ?? WalletReadyState.NotDetected;
